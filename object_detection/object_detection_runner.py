...
label_file=open("./output/label_file.txt","w+")
...
def detect_objects(image_path):
    ...
    for i in range(len(scores[0])):
        if(scores[0][i]>=MINIMUM_CONFIDENCE):
            label_list.add(CATEGORY_INDEX[classes[0][i]]['name'])
            #label_file.write("%s, " % CATEGORY_INDEX[classes[0][i]]['name'])
    for i in label_list:
        label_file.write("%s, "%i)
    label_file.write('\n')
